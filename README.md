...
- 文件双击使用默认应用打开；目录双击进入目录。
- 右键“打开方式”列出 macOS 可用应用，并保证包含“文本编辑”。
...
- 常见图片优先通过 `rayon` 工作线程和 `image` 解码为 256×256 PNG；解码失败及其他格式使用 macOS `qlmanage` 生成缩略图。
...
- 非图片复杂格式依赖 macOS Quick Look 提供缩略图和预览。
